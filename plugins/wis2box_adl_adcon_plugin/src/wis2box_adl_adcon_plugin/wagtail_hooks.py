...
from .views import wis2box_adl_adcon_plugin_index, station_parameter_mapping_list, station_parameter_mapping_create
...
def urlconf_wis2box_adl_adcon_plugin():
    return [
        ...
        path('wis2box_adl_adcon_plugin/station_parameter_mapping/<int:station_mapping_id>/',
             ...
             name='station_parameter_mapping_list'),
...
@hooks.register("register_wis2box_adl_menu_items")
def register_wis2box_adl_menu_items():
    url = reverse("wis2box_adl_adcon_plugin_index")

    menu_items = [
        MenuItem(label=gettext("ADCON Plugin"), url=url, icon_name="cog", order=1000)
    ]
